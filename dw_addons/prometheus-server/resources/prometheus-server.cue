...
prometheusServer: {
	...
	_traits: [{
		...
		properties: {
			name:  "init-config"
			image: "registry.cn-hangzhou.aliyuncs.com/dtweave/curl:8.9.1"
			args: ["sh", "-c", ##"""
		...
		if parameter.customConfig != _|_ {
			...
			properties: {
				...
				image: "registry.cn-hangzhou.aliyuncs.com/dtweave/configmap-reload:v0.5.0"
		...
		if parameter.thanos {
			...
			properties: {
				operations: [{
					...
					value: {
						name:  "thanos"
						image: "registry.cn-hangzhou.aliyuncs.com/dtweave/thanos:v0.8.0"
